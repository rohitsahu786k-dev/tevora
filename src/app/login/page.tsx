...
export const metadata = createPageMetadata({
  ...
  description:
    "Sign in or request access to ONESPACE technical downloads using your ONESPACE account ID.",
  path: routes.login,
});
              ...
              <div className="glass-panel-strong mt-5 p-5">
                ...
                <dl className="type-body-sm text-ink-muted mt-4 grid gap-2">
                  <div>
                    ...
                    <dd>demo@onespace.design</dd>
                  </div>
                  <div>
                    <dt className="text-ink font-semibold">ONESPACE ID</dt>
                    <dd>ONESPACE-DEMO-2026</dd>
                  </div>
                  <div>
...
function LoginPanel({
  ...
  const message =
    status === "success"
      ...
      : status === "demo"
        ? "Demo ONESPACE ID confirmed. Return to Resources to review the signed-in state."
        : status === "database"
          ? "The ONESPACE login database is not connected yet. Configure DATABASE_URL to enable account access."
          : status === "invalid"
            ? "Please enter a valid work email, company and ONESPACE ID."
            : status === "error"
  ...
  return (
    <div className="glass-panel-strong p-6 sm:p-8">
      ...
      <p className="type-body-sm text-ink-muted mt-3">
        Enter your ONESPACE ID for technical download access. Use the test login
        on this page to confirm the signed-in resource flow locally.
      </p>
      {message && (
        <div
          role="status"
      ...
      <form action={requestLoginAccess} className="mt-8 grid gap-5">
        ...
        <label>
          <span className="type-spec-label block">ONESPACE ID</span>
          <input
            name="onespaceId"
